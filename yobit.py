...
API_KEY = '3A43B213966E85C7AF6BD29CAAC528A6' 
API_SECRET = b'65b5bbcf14bb6599d97a20b9853bc25d'
...
CAN_SPEND = 950 # Сколько тратить CURRENCY_2 каждый раз при покупке CURRENCY_1 (100р в моем случае)
    ...
    try:
        ...
        for order in opened_orders:
            if order['type'] == 'sell':
                ...
                raise ScriptQuitCondition('Выход, ждем пока не исполнятся/закроются все ордера на продажу liza_rur (один ордер может быть разбит биржей на несколько и исполняться частями)')
            else:
